...
with st.sidebar:
    ...
    h = st.slider('Tunnel Tiefe (h) [m]', min_value=d, max_value=2500.0, value=60.0, step=1.0, format=None, key=2, help=None, on_change=None, args=None, kwargs=None, disabled=False)
...
ax[1].plot(rm_segment, Krad[rm_segment], label=('Pr = '+str(Krad[rm_segment])), c='r', linestyle='--')
ax[1].plot(rm_segment, Ktang[rm_segment], label=('Pt = '+str(round(Ktang[rm_segment],1))), c='b', linestyle='--')
ax[1].plot(rm_segment, Kschub[rm_segment], label=('Ps = '+str(round(Kschub[rm_segment],1))), c='b', linestyle='--')
